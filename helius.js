...
async function getWalletFundingSource(address, apiKey) {
  try {
    ...
    // Must filter out system programs which appear in every transaction
    ...
    // First pass: find account whose SOL balance decreased significantly (>= 0.001 SOL)
    // and is not a system program and is not the target wallet
    for (let i = 0; i < accounts.length; i++) {
      const pk = accounts[i]?.pubkey || accounts[i];
      if (!pk || pk === address) continue;
      if (SYSTEM_PROGRAMS.has(pk)) continue;
      
      const decrease = (preBalances[i] || 0) - (postBalances[i] || 0);
      // Must have sent at least 0.001 SOL (1,000,000 lamports)
      if (decrease >= 1000000) {
        funderAddress = pk;
        break;
      }
