...
async function getWalletFundingSource(address, apiKey) {
  try {
    // Get all signatures for this address, find the oldest one
    ...
    // Oldest signature is the last in the array
    const oldestSig = sigs[sigs.length - 1].signature;

    // Get the actual transaction to find who funded this wallet
    ...
    // The funder is the first account in the transaction that isn't this wallet
    const accounts = tx.transaction?.message?.accountKeys || [];
    const funder = accounts.find(a => {
      const pk = a.pubkey || a;
      return pk !== address;
    });

    return funder?.pubkey || funder || null;
  } catch {
    return null;
  }
}
