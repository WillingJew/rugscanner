...
app.post('/scan/ai', requireAuth, requirePro, async (req, res) => {
  ...
  try {
    // Run ST data fetch and padre.gg scrape in parallel
    console.log('[AI Scan] Starting parallel fetch for', ca);
    const [tokenData, scrapedFunders] = await Promise.all([
      analyzeTokenST(ca, process.env.SOLANA_TRACKER_API_KEY, process.env.HELIUS_API_KEY, false),
      scrapeFundedBy(ca)
    ]);

    // Merge scraped funder data into holder list
    if (scrapedFunders && scrapedFunders.length > 0) {
      for (const h of tokenData.holders) {
        const scraped = scrapedFunders.find(s => s.rank === h.rank);
        if (scraped) {
          h.funderAddress = scraped.funderAddress || h.funderAddress;
          h.clockNumber = scraped.clockNumber;
          if (scraped.isLP) h.isLP = true;
        }
      }
      console.log('[AI Scan] Merged funder data for', scrapedFunders.length, 'holders');
    }
    const result = detectRisks(tokenData);
