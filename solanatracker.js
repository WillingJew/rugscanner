...
async function getTopHolders(mintAddress, apiKey) {
  // Try paginated endpoint first, fall back to standard
  const res = await fetch(`${ST_API}/tokens/${mintAddress}/holders/paginated?limit=25`, {
    headers: { 'x-api-key': apiKey }
  });
  if (res.ok) return res.json();
  
  // Fallback to standard holders endpoint
  const res2 = await fetch(`${ST_API}/tokens/${mintAddress}/holders`, {
    headers: { 'x-api-key': apiKey }
  });
  if (!res2.ok) {
    console.error(`[ST] Holders fetch failed: ${res2.status} for ${mintAddress}`);
    throw new Error(`Holders fetch error: ${res2.status}`);
  }
  return res2.json();
}
